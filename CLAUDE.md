...
npm test                   # Run mocha tests with babel transpilation
...
Run a single test:
```bash
npx mocha --compilers js:babel-core/register test/TimerPomodoroTest.js
```
...
- Sound files (MP3) are copied to `~/Library/Sounds/` at runtime (macOS-specific)
...
- Break timer shows wrong popup/sound after countdown finishes
- Cannot Ctrl+C terminate during break timer phase
